...
export const entry: TheoryEntry = {
  ...
  answerCapsule:
    "Universal Question Geometry treats a question as an operation that changes the state of knowledge, which means questions can be ranked, ordered, and scored before any are asked. The method keeps a running picture of the current state of knowledge, estimates how much each candidate question would move that picture, and selects the next question from the top of that ranking. Stopping is not allowed on the feeling of being finished; a stop attempt is rejected while a question with positive expected value is still open.",
  summary:
...
Universal Question Geometry starts from a different claim: questions have shape. A question is not a request for a fact. It is an operator that moves a belief state from one position to another. Some moves are large. Some are tiny. Some open doors that other questions can then walk through, which means their real value is not in their own answer but in what they unlock. If that is true, then a set of candidate questions has a landscape, with high ground, low ground, and a frontier of choices where you cannot improve one thing without giving up another.

The implementation makes that landscape computable. It ranks candidate questions over the current state. It records the peaks, the valleys, and the frontier. When an answer comes back, it updates the beliefs and the causal structure underneath them, then re-ranks.

## Three rules do most of the work

**Answers must be qualified, not tagged.** A challenge pass is not complete because someone marked it complete. It is complete when the answers meet the conditions.
...
**Depth is typed, not counted.** The familiar "five whys" rule stops at five because five is a memorable number. Here, the traversal continues by the type of thing you are standing on. You stop when you reach bedrock: a primary source, a named mechanism, or a human ruling. Not when you get tired.

## Stopping has guards

Residual expected value of information, decision robustness, whether a question was actually selected by the runtime, and whether any positive-value question remains. A rejected stop exits with code 2 and hands back the next required question. That is the part that changes behaviour most: a person who wants to be finished cannot simply declare it, because the runtime is holding an open question with a positive score and will say so.
...
The project's own benchmark standard demands at least 21 retrospective cases across six task families before any performance claim, and batch 01 stands at 52 of 90 predeclared run slots. The repository states in writing that the included benchmark is a smoke test and establishes nothing. So the honest state of this theory is: developed, and deliberately unproven. The machinery is real, the discipline around the machinery is real, and the result is outstanding.

That gap is stated here rather than papered over, because the whole point of a theory page is to be checkable. A reader who wants to disagree with this one has everything needed to do it: the runnable loop, the standard it will be judged against, and the count of how far the evidence has actually got.
