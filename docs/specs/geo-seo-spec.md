...
| 8 | **No `h3ro` branding** | `grep -rniE "h3ro" app/ lib/ content/ public/ .next/server/app/` returns **only** the two allowlisted profile URLs `https://x.com/h3roai` and `https://www.tiktok.com/@h3ro.ai`. Any other hit — `h3ro.ai` as a domain, `h3ro-dev.github.io` proof links, "h3ro" in copy or a component name — fails. The allowlist is explicit and lives in the script; a naive grep would fail on the permitted links, so this check must never be "simplified" |
