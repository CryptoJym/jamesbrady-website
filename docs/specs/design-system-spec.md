...
| **Buttons** | `.btn` 44px, mono `--ts-small`, `--track-label`, `--r-sm`. `.btn--primary` = `--sig-btn` fill with `--t-hi` ink (6.93:1). Dark ink on neon `--sig` is banned for buttons — chromostereopsis, not just a ratio. `.btn--ghost` = `--c-edge-int` border on `--c-panel`. Hover: -1px translate + `--sig-btn-hi`. `.ask` nav pill is 36px (44px ≤620px). |
...
| **Skip link** (`.skip`) | Fixed, off-screen until `:focus`, 44px, `--sig-btn` fill with `--t-hi` ink. First element in `<body>`; target `<main id="main" tabindex="-1">`. |
